...
if __name__ == '__main__':
    ...
    from detect_merges_splB_z0 import test_paths_cache_folder
    ...
    grid_search = [0.001, 0.01, 0.1, 0.2, 0.5, 1, 2, 5, 10, 100, 1000]

    for val in grid_search:

        # Set the path weights factor
        ExperimentSettings().lifted_path_weights_factor = val

        # Local resolving ---------------------
        # TODO: Change here when adding result
        new_nodes_filepath = os.path.join(meta_folder, 'new_ones_local_gs_{}.pkl'.format(val))
        ...
        # TODO: Change here when adding result
        result_filepath = os.path.join(experiment_folder, 'result_resolved_local_gs_{}.h5'.format(val))
